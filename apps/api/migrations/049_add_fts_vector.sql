...
BEGIN
  NEW.fts_vector := to_tsvector('english',
    coalesce(NEW.label, '') || ' ' || coalesce(NEW.type, '')
  );
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER iris_entities_fts_trigger
  BEFORE INSERT OR UPDATE OF label, type ON iris_entities
...
SET fts_vector = to_tsvector('english', coalesce(label, '') || ' ' || coalesce(type, ''))
