 ...
 * @returns Hono router for context summarization endpoints
 */
export function createContextSummarizationRoutes(sql: Sql): Hono {
  const app = new Hono();

  /** POST /summarize — generate task-specific summary */
  ...
  /** GET /metrics — aggregated compression metrics */
  ...
  /** POST /metrics/track — record a summarization event */
  ...
  /** POST /classify-intent — classify a query's intent */
...
export default createContextSummarizationRoutes(undefined as unknown as Sql);
