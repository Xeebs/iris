...
export function createQueryClusteringRoutes(sql: Parameters<typeof clusterQueriesBySemanticSimilarity>[0]) {
  const app = new Hono();
  ...
  app.get('/clusters/:clusterId', async (c) => {
    ...
    const cluster = allResult.value.find(cl => cl.clusterId === clusterId);
    if (!cluster) {
