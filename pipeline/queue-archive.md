...
## Layer 79 — Slice 2
### Task: Derive workspaceId from the authenticated MCP API key
- **Layer**: 79 — Slice 2
- **Status**: COMMITTED
- **Priority**: High
- **Description**: Found during the Slice 2 owner verification session (2026-06-12): every MCP tool requires a `workspaceId` parameter, but there is no tool or MCP resource to discover it — the owner's client had to query Postgres directly for the workspace ID, an escape hatch no real external client has. The API key is already scoped to exactly one workspace (`mcp_api_keys.workspace_id`), so the server should resolve the workspace from the authenticated key. Implementation: (1) in the auth layer, resolve and attach `workspace_id` from the key row at connection/request time; (2) make `workspaceId` optional in every tool's zod schema and inputSchema, defaulting to the authenticated key's workspace — registration goes through `register-tool.ts`, so thread the default there rather than editing 20 call sites where possible; (3) if a caller passes a `workspaceId` that differs from the key's workspace, return a structured error (tools never throw); (4) update `docs/CONNECT_CLAUDE.md` and `scripts/mcp-smoke.ts` to stop requiring the workspace ID; (5) unit tests for default-from-key, explicit-match, and mismatch-rejection paths. This unblocks the owner sign-off criterion in docs/SLICE_2.md — the live-session experience is the criterion under test.
- **Files**:
  - apps/mcp-server/src/server.ts (auth/key resolution)
  - apps/mcp-server/src/register-tool.ts
  - apps/mcp-server/src/tools/*.ts (schemas only where the helper cannot cover)
  - docs/CONNECT_CLAUDE.md
  - scripts/mcp-smoke.ts
- **Depends on**: nothing
- **Added**: 2026-06-12
