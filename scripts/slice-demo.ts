 ...
 * entity counts in the vector store. Fails fast with a clear message at the
 * first broken step; exit 0 means the connect → sync → index chain works.
 *
...
async function main(): Promise<void> {
  step('1/7 Reset database (drop + recreate schema)');
  ...
  step('3/7 Start API server + sync worker (DEMO_MODE, hash-deterministic embeddings)');
  ...
  step('4/7 Bootstrap workspace + MCP API key');
  ...
  step('5/7 Create demo-mode HubSpot connector instance');
  ...
  step('6/7 Trigger sync and wait for completion');
  ...
  step('7/7 Verify indexed entity counts per type');
  ...
  console.log('\nSLICE DEMO (connect → sync → index): PASS');
  for (const child of children) child.kill('SIGTERM');
  process.exit(0);
}
